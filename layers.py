...
class ReLuLayer(Layer):
    ...
    def forward(self, buffer):
        ...
        return np.where(buffer < 0, 0.0, buffer*0.015)

    def backward(self, input, buffer):
        dad=np.where(input < 0, 0.01, 1)*buffer
        return np.where(input < 0, 0.0, 0.015)*buffer
...
class ConvolutionLayer(Layer):
    def __init__(self, config):
        ...
        self.filter =np.random.uniform(-1, 1,size=sz)
...
class SoftmaxLossLayer(Layer):
    ...
    def loss(self, predicted, expected):
        ...
        output = np.sum(predicted*expected)*1.0/len(predicted)
